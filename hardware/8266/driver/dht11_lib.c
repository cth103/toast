...
//-----------------------------------------------------------------------------------------
LOCAL void ICACHE_FLASH_ATTR
dht11_protocol(uint32 gpio_status, int cause)
{
...
// Reactivate interrupts for GPIO0
            gpio_pin_intr_state_set(GPIO_ID_PIN(dht11_gpio), GPIO_PIN_INTR_ANYEDGE);
...
dht11_gpio_intr_handler(uint32 gpio_status)
{
...
//-----------------------------------------------------------------------------------------
void ICACHE_FLASH_ATTR
dht11_init(int gpio_id,uint32_t user_task,uint32_t user_task_signal)
{
    dht11_gpio = gpio_id;
...
// set gpio to  0 for 18 mS minimun
    ...
    os_timer_disarm(&dht11_timer);
    os_timer_arm(&dht11_timer,20,arg);
}
